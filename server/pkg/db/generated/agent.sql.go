...
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, trigger_source, mode, parent_task_id, swarm_role
`
...
VALUES ($1, $2, $3, 'queued', $4, $5, $8, $6, $9, $7)
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, trigger_source, mode, parent_task_id, swarm_role
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, trigger_source, mode, parent_task_id, swarm_role FROM agent_task_queue
WHERE id = $1
`
...
const getLatestTaskCreatedAtByIssueAgentSource = `-- name: GetLatestTaskCreatedAtByIssueAgentSource :one
...
type GetLatestTaskCreatedAtByIssueAgentSourceParams struct {
	IssueID       pgtype.UUID `json:"issue_id"`
	AgentID       pgtype.UUID `json:"agent_id"`
	TriggerSource string      `json:"trigger_source"`
}

func (q *Queries) GetLatestTaskCreatedAtByIssueAgentSource(ctx context.Context, arg GetLatestTaskCreatedAtByIssueAgentSourceParams) (pgtype.Timestamptz, error) {
	row := q.db.QueryRow(ctx, getLatestTaskCreatedAtByIssueAgentSource, arg.IssueID, arg.AgentID, arg.TriggerSource)
...
const listChildTasksByParent = `-- name: ListChildTasksByParent :many
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, trigger_source, mode, parent_task_id, swarm_role FROM agent_task_queue
WHERE parent_task_id = $1
ORDER BY created_at ASC
`

func (q *Queries) ListChildTasksByParent(ctx context.Context, parentTaskID pgtype.UUID) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listChildTasksByParent, parentTaskID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listRunnableAgents = `-- name: ListRunnableAgents :many
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, tools, triggers, runtime_id, instructions, archived_at, archived_by FROM agent
WHERE archived_at IS NULL AND runtime_id IS NOT NULL
ORDER BY created_at ASC
`

func (q *Queries) ListRunnableAgents(ctx context.Context) ([]Agent, error) {
	rows, err := q.db.Query(ctx, listRunnableAgents)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
